...
const TrainingEmployeeTable = ({ employees = [], loading, error, onEditClick, onStatusChange }) => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [selectedEmployee, setSelectedEmployee] = useState(null);
  const [openStatusPopper, setOpenStatusPopper] = useState(false);

  const trainingEmployees = employees?.filter(emp => 
    emp?.status?.toLowerCase() === 'training'
  ) || [];
  ...
  if (loading) return <div className="loading" style={{ fontSize: '0.9rem' }}>Loading training employees...</div>;
  if (error) return <div className="error" style={{ fontSize: '0.9rem' }}>Error: {error}</div>;
  if (trainingEmployees.length === 0) return <div className="no-data" style={{ fontSize: '0.9rem' }}>No training employees found</div>;
