...
const ResignedEmployeeTable = ({ employees = [], loading, error, onEditClick, onStatusChange }) => {
  ...
  if (loading) return <div className="loading" style={{ fontSize: '0.9rem' }}>Loading resigned employees...</div>;
  if (error) return <div className="error" style={{ fontSize: '0.9rem' }}>Error: {error}</div>;
  if (resignedEmployees.length === 0) return <div className="no-data" style={{ fontSize: '0.9rem' }}>No resigned employees found</div>;
      ...
      <table style={{ 
        ...
        <thead>
          <tr style={{ 
            ...
            <th scope="col" style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>Date Resigned</th>
