...
const FloatingEmployeeTable = ({ employees = [], loading, error, onEditClick, onStatusChange }) => {
  ...
  if (loading) return <div className="loading" style={{ fontSize: '0.9rem' }}>Loading floating employees...</div>;
  if (error) return <div className="error" style={{ fontSize: '0.9rem' }}>Error: {error}</div>;
  if (floatingEmployees.length === 0) return <div className="no-data" style={{ fontSize: '0.9rem' }}>No floating employees found</div>;
  ...
  return (
    <div style={{ 
      ...
      <table style={{ 
        ...
        <tbody>
          {floatingEmployees.map((employee) => (
...
export default FloatingEmployeeTable;