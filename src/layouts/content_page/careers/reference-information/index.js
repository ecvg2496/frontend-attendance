...
import { useMaterialUIController, setDialog, setLoading } from "context";
import ReferenceForm from "./reference";

function ReferenceInformation({ update, heightUpdate }){
    const [controller, dispatch] = useMaterialUIController();
    const { dialog, loading } = controller;
    // navigation
    ...
    return (
        <MDBox>
            ...
            <MDTypography my={1} sx={{ display: ref && (Object.keys(ref).length >= 2) ? 'none' : 'block' }} color='error' variant='button'>Add at least 2 references</MDTypography>
        </MDBox>
    );
}
