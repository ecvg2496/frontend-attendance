            ...
            {({ TransitionProps, placement }) => (
                ...
                >
                    <Paper>
                        ...
                        <MenuList autoFocusItem={state.open} onKeyDown={handleListKeyDown}>
                            {
                                content && Object.keys(content)
                                .filter(item => ["Rejected", "Failed"].includes(content[item].title)) // Only keep "Rejected" & "Failed"
                                .map((item, key) => {
                                    const badge = content[item];

                                    return (
                                        <MenuItem 
                                            key={key} 
                                            onClick={(e) => handleClose(badge.id)}
                                            sx={{ 
                                                bgcolor: 'transparent', 
